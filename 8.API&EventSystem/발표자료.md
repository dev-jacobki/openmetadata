...
성공한 C/U/D 응답은 Java의 `EventFilter`가 잡아 비동기 `ChangeEventHandler`로 넘긴다. 따라서 ChangeEvent 저장은 원래 API 처리와 분리되어 있으며, 저장 실패가 이미 반영된 엔티티 변경이나 API 성공 응답을 되돌리지는 않는다.
...
`ChangeEventHandler`에서 `change_event` INSERT가 실패하면 오류 로그를 남기고 처리를 끝낸다. 이때 ChangeEvent 행과 `offset`은 만들어지지 않으므로 Java Scheduler가 해당 Event를 조회할 수 없고, Subscription 목적지 전송과 ChangeEvent 기반 감사 이력에도 남지 않는다. 이를 보여 주는 전용 UI 상태·재시도 버튼·실패 Event 대기열은 없으며, OpenMetadata가 INSERT를 자동 재시도하거나 이미 성공한 엔티티 C/U/D를 롤백하지도 않는다.

따라서 운영 담당자가 서버 로그·모니터링으로 실패를 확인해야 한다. 원래 C/U/D를 그대로 다시 보내도 이미 같은 상태라면 `entityNoChange`가 되어 새 ChangeEvent가 생기지 않을 수 있다. 누락된 이벤트가 반드시 필요하면 해당 변경을 별도로 대사·복구하는 절차를 두고, ChangeEvent를 직접 DB에 넣기보다는 안전한 API 변경 또는 외부 보정 처리로 다룬다.
