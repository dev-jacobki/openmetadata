...
호출 제한·timeout 뒤 재시도 기준은 1.7에서 다룬다. Ingestion Python Client에는 재시도 횟수와 대기 시간을 설정하는 옵션이 있지만, curl·Java SDK·자체 스크립트가 자동으로 같은 정책을 적용해 주지는 않는다.
...
### 3.4 Subscription의 시작점과 목적지

3.1의 흐름처럼 `ChangeEvent`는 변경 사실을 남긴 데이터이고, Event Subscription은 그 이벤트의 수신 규칙을 담은 설정이다. 새 Subscription은 생성 시점의 최신 offset에서 시작하므로 등록 **이후** 생성된 이벤트만 기본 전달한다. 이미 실행된 Subscription은 저장된 offset부터 다시 시작한다.
...
새 EventType 추가 절차는 3.3을 따른다. 임의 Header를 넣어도 ChangeEvent에 새 필드가 자동 저장되지는 않으므로, Change Request ID처럼 표준 스키마에 없는 업무 값은 API 실행 기록과 Webhook 수신 기록에서 연결한다.
...
Bot을 가진 자동화 스크립트가 OpenMetadata API를 호출하는 예시다. 변경 결과는 앞 절의 Event Subscription으로 별도 관측할 수 있다.
...
이 예시는 5.1의 API 호출 결과가 Event System에서 어떻게 보이는지 확인하는 방식이다. 수신기 구성과 재시도 정책은 3.8의 Webhook 흐름을 따른다.
...
1. Table Update 이벤트용 Subscription에 Generic Webhook의 HTTPS 주소와 인증 Header를 등록한다.
2. 5.1처럼 Table Description이나 Tag를 수정한다.
3. 수신기는 `id`, `eventType`, `entityId`, `timestamp`를 기록하고, 선택 값인 `changeDescription`, FQN, `userName`은 없을 수 있게 처리한다.
4. 같은 `event.id`가 이미 있으면 후속 처리를 생략하고 `2xx`를 반환한다.
