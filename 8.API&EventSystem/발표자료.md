...
대량 호출이 들어오면 OpenMetadata는 목록 요청과 Bulk 변경 요청을 다르게 처리한다. 목록 API는 한 요청에 한 페이지만 반환하고, Bulk API는 동기 처리 또는 Worker 대기열 처리를 선택할 수 있다.

| 대량 호출 상황 | OpenMetadata의 반응·대비 | 호출자에게 보이는 결과 |
|---|---|---|
| 목록 조회가 많음 | `limit`과 Cursor로 한 요청의 응답 크기를 제한한다. 다음 페이지는 호출자가 `after`를 보내야 조회한다. | 한 번에 전체 목록을 쌓아 처리하지 않고, 페이지 단위 응답을 받는다. |
| Bulk C/U/D — 기본 | 요청을 검증하고, 필요한 부모·기존 엔티티를 묶어 조회한 뒤 요청을 처리한다. | 처리 완료까지 연결을 유지하고 `200`과 항목별 성공·실패 결과를 받는다. |
| Bulk C/U/D — `?async=true` | 검증을 마친 작업을 제한된 Bulk Worker에 넣고, Worker가 가능한 순서에 처리한다. | 즉시 `202 Accepted`를 받고, 실제 반영은 뒤에서 진행된다. |
| Worker가 바쁨 | 기본값은 Worker 10개, 대기열 1,000개다. 동시에 허용하는 Bulk Job 수도 제한한다. | 처리 시작이 늦어질 수 있고, 동시 Bulk Job 한도(100개)에 도달하면 `429 Too Many Requests`로 받지 않는다. |
| DB·검색 부하가 큼 | OpenMetadata 자체에 공통 RPS 제한은 없으며, Worker 수·대기열로 Bulk 부하를 제한한다. 배포 환경에서는 Gateway·Ingress에도 별도 제한을 둘 수 있다. | 응답 시간이 길어지거나 앞단 제한을 받을 수 있다. |

호출자는 목록은 Pagination으로 나누고, 대량 C/U/D는 가능한 Bulk API로 묶되 실제 변경 항목만 보낸다. 동시에 너무 많은 Bulk Job을 열기보다 낮은 동시성으로 시작해 Worker 대기와 DB 부하를 관찰하며 조절한다.
