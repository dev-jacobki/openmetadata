...
대량 호출에서는 서버가 한 번에 처리하는 양과 호출자가 보내는 양을 나누어 본다.

##### OpenMetadata의 대량 처리

| 상황 | OpenMetadata의 처리·대비 |
|---|---|
| 대량 목록 조회 | `limit`과 cursor/offset 기반 Pagination으로 결과를 나누어 반환 |
| Table Bulk Create or Update | `PUT /v1/tables/bulk`가 항목별 성공·실패 결과를 반환하고, 설정에 따라 비동기 처리할 수 있음 |
| Bulk 처리량 | 서버의 `bulkOperation.maxThreads`, `queueSize`, `timeoutSeconds`가 병렬 처리·대기열·최대 시간을 제어 |
| 대기열 포화 | 작업을 더 받지 않고 `503`을 반환할 수 있음 |
| 호출 제한 | OpenMetadata 공통 RPS 제한은 없으며, 실제 한도는 서버 DB·검색 부하와 앞단 API Gateway·Ingress 설정에 따라 달라짐 |

##### 호출 측 최적화

| 호출 방식 | 최적화 방법 |
|---|---|
| 대량 조회 | 필요한 `fields`만 요청하고, `limit`을 작게 시작해 페이지 단위로 처리 |
| 대량 C/U/D | 원본과 현재 상태를 비교해 실제 변경 항목만 호출하고, 항목별 성공·실패를 기록 |
| 병렬 호출 | 낮은 동시성부터 시작하고 같은 엔티티에는 동시에 쓰지 않음 |
| 중단 뒤 재개 | 마지막 cursor 또는 처리한 UUID를 기록해 이미 성공한 항목을 다시 쓰지 않음 |

호출 제한·timeout 뒤 재시도 기준은 1.7에서 다룬다.
...
| 상태 | 의미 | 재시도 방법 |
|---:|---|---|
| `200`, `201` | 요청 성공 | 재시도하지 않고 UUID·FQN·Version을 기록 |
...
### 1.8 새로운 API를 추가할 때: Table API를 기준으로

새 API는 경로만 추가하는 작업이 아니다. 요청·응답 스키마, 권한, 저장 방식, 이벤트 영향을 함께 정한다. Table API는 이 흐름을 확인하기 좋은 기준 구현이다.

| 처리할 항목 | Table 기준 구현 위치 | 새 API에서 확인할 내용 |
|---|---|---|
| 엔티티·요청 모델 | `openmetadata-spec/src/main/resources/json/schema/entity/data/table.json`<br>`openmetadata-spec/src/main/resources/json/schema/api/data/createTable.json` | 요청 필수값, 응답 모델, UUID·FQN 식별 방식 |
| REST 경로·메서드 | `openmetadata-service/src/main/java/org/openmetadata/service/resources/databases/TableResource.java` | `@Path`, HTTP 메서드, Path·Query Parameter, 상태 코드 |
| 공통 C/U/D·권한 | `openmetadata-service/src/main/java/org/openmetadata/service/resources/EntityResource.java` | Create/Update/Delete 권한과 `PUT`·`PATCH`·복구 동작을 공통 규칙으로 쓸 수 있는지 |
| 저장·관계 처리 | `openmetadata-service/src/main/java/org/openmetadata/service/jdbi3/TableRepository.java` | DB 저장, 관계·FQN·버전·Soft Delete 처리 |
| ChangeEvent 영향 | `openmetadata-service/src/main/java/org/openmetadata/service/events/EventFilter.java`<br>`openmetadata-service/src/main/java/org/openmetadata/service/events/ChangeEventHandler.java` | 성공 응답 뒤 이벤트가 만들어지는지, 새 응답 형식에 맞는 ChangeEvent가 생성되는지 |

기존 Table에 하위 API를 추가한다면 `TableResource`에서 경로와 입력을 정의하고, 필요한 저장 로직은 `TableRepository`에 둔다. 완전히 새로운 메타데이터 엔티티라면 먼저 스키마와 Create 요청 모델을 만들고, 그 모델을 사용하는 Resource·Repository·권한·테스트를 함께 추가한다. 구현 뒤에는 정상·권한 거부·충돌·삭제·ChangeEvent까지 API 계약으로 확인한다.
...
SSO는 사람의 로그인 경로에 추가되는 인증 방식이다. API 권한 판단은 SSO Token 자체가 아니라, 로그인 뒤 OpenMetadata가 발급한 JWT와 Role·Policy로 수행된다.
...
Event Subscription은 “어떤 이벤트를 받는가”뿐 아니라 **언제부터**와 **어디로** 전달할지를 정한다. 이 기준이 없으면 새 구독을 만들 때 과거 변경까지 알릴지, 장애 뒤 어느 위치부터 다시 보낼지가 불명확해진다.
...
| 목적지 | 아래 Destination 설정을 사용 | 알림 채널과 외부 시스템 수신 경로를 결정 |
...
|---|---|---|
| 구독 조건·수신자·실행 주기 | `openmetadata-spec/src/main/resources/json/schema/events/eventSubscription.json`<br>`openmetadata-service/src/main/java/org/openmetadata/service/resources/events/subscription/EventSubscriptionResource.java` | 기존 `filteringRules`, `destinations`, `pollInterval`, `retries` 설정을 API·UI에서 구성 |
| 새 Filter Resource·함수 | `openmetadata-service/src/main/java/org/openmetadata/service/events/subscription/EventsSubscriptionRegistry.java`<br>`openmetadata-service/src/main/java/org/openmetadata/service/events/subscription/AlertsRuleEvaluator.java` | UI/API에 노출할 Resource Descriptor와 조건 평가 함수를 등록 |
| 새 전달 채널 | `openmetadata-service/src/main/java/org/openmetadata/service/apps/bundles/changeEvent/AlertFactory.java` | Destination type과 `Destination<ChangeEvent>` 구현을 추가해 새 채널로 전달 |
| 구독 처리 자체 교체 | `openmetadata-service/src/main/java/org/openmetadata/service/apps/bundles/changeEvent/AbstractEventConsumer.java` | Consumer subclass를 구현하고 Event Subscription의 `className`에 등록 |
...
기존 목적지의 메시지 내용만 바꾸거나 사내 업무 조건을 추가하는 경우에는 서버 파일을 수정하기보다 Generic Webhook 수신기에서 처리하는 편이 운영 부담이 작다. 임의 Header를 넣어도 ChangeEvent에 새 필드가 자동 저장되지는 않는다.
