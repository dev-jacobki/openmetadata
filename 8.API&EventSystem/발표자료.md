...
`ChangeEventHandler`에서 `change_event` INSERT가 실패하면 서버 오류 로그만 남기며, 이를 보여 주는 전용 UI 상태나 재시도 버튼은 없다. OpenMetadata는 INSERT를 자동 재시도하지 않고, 이미 성공한 엔티티 C/U/D도 롤백하지 않는다.
