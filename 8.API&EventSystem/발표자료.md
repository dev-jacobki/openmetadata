...
대량 호출은 Pagination만의 문제가 아니다. **호출자는 요청을 나누어 보내고, OpenMetadata는 받은 요청을 인증·처리·대기·응답한다.** 역할을 분리해 봐야 처리량과 장애 원인을 판단할 수 있다.

#### OpenMetadata 서버가 하는 일

| 상황 | Java 처리 위치 | OpenMetadata가 하는 일 |
|---|---|---|
| API 요청 전 인증 | `security/DelegatingContainerRequestFilter` | Resource 메서드가 실행되기 전에 현재 인증 Filter로 요청을 넘겨 Token을 확인 |
| 대량 목록 조회 | `TableResource` → `EntityResource.listInternal` → `TableRepository` | `limit`, `after`, `offset`으로 한 페이지를 조회하고 다음 Cursor 또는 목록을 반환 |
| Table Bulk Create or Update | `TableResource.bulkCreateOrUpdate` → `EntityResource.processBulkRequest` | `PUT /v1/tables/bulk` 요청을 항목별로 처리하고 성공·실패 결과를 반환 |
| Bulk 작업 처리량 | `bulkOperation` 설정과 Bulk Worker | 병렬 Worker 수, 대기열 크기, 최대 처리 시간을 적용 |
| 대기열 포화 | Bulk Worker 대기열 | 더 처리할 여유가 없으면 작업을 받지 않고 `503`을 반환할 수 있음 |
| 요청 속도 제한 | 서버·앞단 Gateway/Ingress | OpenMetadata 자체에는 공통 RPS 제한이 없으며, 실제 한계는 DB·검색 부하와 앞단 설정에 따라 달라짐 |
...
기존 Table에 하위 API를 추가한다면 `TableResource`의 비슷한 메서드부터 확인한다. 완전히 새로운 엔티티라면 스키마·요청 모델, Resource·Repository, 권한과 ChangeEvent 영향 범위를 같은 순서로 연결한다. `PUT`의 Upsert 의미는 Table API의 계약이므로 다른 API에 자동으로 복사하지 않는다.
...
`ChangeEventHandler`에서 `change_event` INSERT가 실패하면 오류를 기록하고 해당 저장만 실패한다. 이 INSERT 자체를 자동으로 다시 시도하거나, 이미 성공한 엔티티 C/U/D를 롤백하지는 않는다. 따라서 ChangeEvent 저장 실패는 서버 오류 로그와 `change_event` 증가량으로 별도 관측해야 한다. 이는 뒤의 Subscription 목적지 전송 재시도와는 다른 단계다.
...
Subscription은 UI에서도 만들 수 있다. **Settings → Notifications → Alerts**에서 Alert를 추가해 Filter·Recipient·Destination을 설정하면 같은 Event Subscription API가 호출된다. 자동화나 배포 설정으로 관리하려면 `POST /api/v1/events/subscriptions` API를 사용한다.
