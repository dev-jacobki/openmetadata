...
Event System은 OpenMetadata의 변경 결과를 기록하고, 필요한 이벤트를 내부 사용자나 외부 시스템에 전달하는 후처리 구조다. 일반 사용자가 이벤트를 직접 생성하는 것이 아니라 UI, Ingestion, SDK가 수행한 변경에서 서버가 이벤트를 만든다.
...
### 3.2 이벤트는 어디에서 만들어지는가

주요 엔티티 C/U/D는 다음 순서로 처리된다.

1. Resource와 Repository가 엔티티 변경을 수행하고 `changeDescription`을 계산한다.
2. 응답이 `201 Created`이거나 변경 유형 응답 헤더를 포함하면 서버의 응답 처리기가 EventType을 결정한다.
3. 응답 엔티티, 로그인 주체, 버전, 변경 내용을 조합해 `ChangeEvent`를 만든다.
4. 민감 필드를 마스킹한 뒤 서버 DB의 `change_event`에 저장한다.
5. Event Subscription이 자신의 offset 이후 이벤트를 비동기로 읽는다.

`PUT` 요청이 현재 상태와 완전히 같으면 응답은 `entityNoChange`가 될 수 있다. 이 유형은 변경 이력용 `change_event`에는 저장되지 않으므로 “No Change 알림”을 구독해도 전달되지 않는다.

API 변경과 이벤트 전달은 한 번의 동기 호출이 아니다. API 응답이 성공한 뒤 구독 스케줄러가 이벤트를 읽어 목적지로 보내므로, API 성공 여부와 이벤트 기록·전달 여부를 각각 관리해야 한다.
