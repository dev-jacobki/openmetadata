...
| 분류 | 무엇을 담나 | 예 | 화면을 만들 때의 의미 |
|---|---|---|---|
| `base` | 하나의 UI 동작을 맡는 기본 부품 | Button, Input, Select, Checkbox | 화면에서 필요한 가장 작은 상호작용을 제공한다. |
| `application` | 여러 기본 부품을 조합한 업무 화면용 부품 | Table, Tabs, DatePicker, Modal | 목록·탭·팝업처럼 화면에 바로 배치할 수 있는 기능 단위다. |
| `foundations` | 제품 전체에서 같은 표현을 유지할 디자인 기반 부품 | Typography, Featured Icon | 글자·아이콘처럼 반복되는 표현의 모양과 의미를 통일한다. |

예를 들어 Table은 `application`에 있지만 내부적으로 Button이나 Icon을 쓸 수 있다. 반대로 모든 Button이 Table을 거쳐야 하는 것은 아니다. 이 분류의 목적은 조립 순서를 강제하는 것이 아니라, 새 컴포넌트를 어느 수준에 추가하고 재사용할지 판단하게 하는 것이다. 패키지의 `components/index.ts`는 세 분류를 모두 다시 export하므로 앱에서는 필요한 컴포넌트를 바로 import한다.
