...
새 위젯 추가 위치
├── openmetadata-ui/
│   └── src/main/resources/ui/src/
│       ├── components/MyData/Widgets/<NewWidget>/
│       │   ├── <NewWidget>.component.tsx       # 데이터 조회·화면 렌더링
│       │   ├── <new-widget>.less               # 위젯 전용 스타일
│       │   └── <NewWidget>.test.tsx            # 위젯 동작 테스트
│       ├── enums/CustomizablePage.enum.ts      # LandingPageWidgetKeys에 FQN 키 추가
│       ├── utils/CustomizeMyDataPageClassBase.ts # 컴포넌트·높이·이미지 매핑
│       └── assets/img/<new-widget>.png          # Add Widget 카드 이미지
└── openmetadata-service/
    └── src/main/resources/json/data/document/docs/
        └── <newWidget>.json                    # 이름·설명·FQN·허용 grid 크기
```

기존 구현은 `DataAssetsWidget.component.tsx`·`dataAsset.json`·`data-assets-widget.png` 또는 `KPIWidget.component.tsx`·`kpi.json`·`kpi-widget.png` 묶음을 따라가면 된다.
...
구현 흐름은 **React 위젯 → 위젯 키 → 렌더링 매핑 → `KnowledgePanel` 문서·미리보기 → Add Widget 선택 → layout 저장** 순서다.
