...
세 폴더는 Atomic Design의 atom → molecule → organism을 그대로 구현한 엄격한 계층은 아니다. **`base`와 `foundations`는 같은 낮은 재사용 수준의 공통 부품이며, 서로 보완한다.** 그 위에서 `application`이 이 부품들을 조합해 Table·Modal·Navigation 같은 화면 단위 UI를 만든다.

차이는 높이가 아니라 역할이다. `base`는 Button·Input처럼 입력·선택·클릭을 담당하는 **기능·상호작용 중심 부품**이고, `foundations`는 Typography·Dot·Featured Icon처럼 글자·아이콘·강조를 담당하는 **시각 표현 중심 부품**이다. 어느 한쪽이 다른 쪽의 부모는 아니다. 실제로 `base/Alert`는 `foundations/FeaturedIcon`을 쓰고, `foundations/Typography`는 `base/Tooltip`을 쓴다. 즉, 필요한 기능에 표현을 붙이고, 표현에 필요한 보조 기능을 붙이는 식으로 함께 쓴다.

![공유 UI 컴포넌트의 재사용 수준과 역할](발표자료-assets/core-components-composition.svg)

| 분류 | 실제 역할 | Atomic Design에 굳이 비유하면 | 주의할 점 |
|---|---|---|---|
| `base` | 여러 화면에서 반복하는 입력·선택·버튼·작은 레이아웃 부품 | atom이 중심이지만 InputGroup·Card·Form처럼 작은 molecule도 섞여 있다. | 낮은 재사용 수준의 기능·상호작용 부품 |
| `application` | base·foundations를 조합해 화면에 바로 놓는 Table·Modal·Navigation·Tabs | organism 또는 화면 패턴에 가장 가깝다. | 전체 Page 자체는 아니며, 특정 업무 화면에서 재사용할 조합 |
| `foundations` | 글자·점·강조 아이콘처럼 공통 시각 표현을 의미 있게 만드는 부품 | Atomic Design의 organism에 대응하지 않는다. 디자인 token·표현 규칙에 가깝다. | 낮은 재사용 수준의 시각 표현 부품 |

정리하면 **`base`는 기능, `foundations`는 표현을 맡는 낮은 수준 공통 부품이고 둘은 서로 보완한다. `application`은 둘을 조합해 화면에 놓는 상위 부품**이다. 이 분류의 목적은 조립 순서를 강제하는 것이 아니라, 새 컴포넌트를 어디에 추가하고 재사용할지 판단하게 하는 것이다. 패키지의 `components/index.ts`는 세 분류를 모두 다시 export하므로 앱에서는 필요한 컴포넌트를 바로 import한다.
...
Untitled UI는 외부 디자인 시스템 제품이다. 이 저장소가 직접 설치해 쓰는 것은 그중 **`@untitledui/icons` 아이콘 패키지**다. Button·Input·Table 전체를 외부 Untitled UI 라이브러리에서 가져오는 구조는 아니며, 실제 컴포넌트는 `@openmetadata/ui-core-components`가 구현한다.

이름이 비슷한 두 항목은 역할이 다르다.

| 항목 | OpenMetadata에서의 역할 |
|---|---|
| `@untitledui/icons` | 외부 React 아이콘 패키지. Button·Featured Icon 등에 아이콘 그림을 표시한다. |
| `UntitledUIThemeProvider` | OpenMetadata 내부 Provider. Theme 설정의 색상을 새 공유 UI가 읽는 CSS 변수로 바꾼다. 아이콘 패키지의 Provider가 아니다. |

`UntitledUIThemeProvider`라는 이름은 새 공유 UI가 Untitled UI 스타일의 색상 이름·표현 기준을 따르기 때문에 붙었다. 아이콘에 Theme가 있다는 뜻은 아니다. 실제 색상 전달 경로는 3.2절에서 본다.
...
### 2.3 `switch/case`: 선택한 Service를 Schema에 연결하는 일반 TypeScript 조건문

`switch/case`는 맞다. **JavaScript에서 쓰는 그 조건문을 TypeScript 파일에서 쓴 것**이다. 문법 자체를 공부하는 절은 아니다. 이 화면에서는 사용자가 고른 Service와 해당 연결 Form의 JSON Schema를 이어 주는 등록 지점이라서 확인한다.

JSON Schema 파일은 “MySQL 연결 화면에는 어떤 입력란이 있는가”만 선언한다. 사용자가 `Database > MySQL`을 골랐을 때 그 파일을 자동으로 찾아오지는 않는다. `switch/case`가 먼저 큰 분류를 고르고, 그 안에서 제품별 Schema를 고른다.
...
따라서 이 절에서 볼 점은 `switch` 문법이 아니라 **선택값 → Schema의 연결표**다. Database 계열에 MySQL과 PostgreSQL을 추가하면 같은 1차 경로를 지나고 2차 분기에서 서로 다른 Schema를 고른다. JSON Schema가 Form의 내용을 정의하고, `switch/case`는 선택된 Service에 맞는 Schema를 찾아 Form에 넘긴다.

### 2.4 Follow-along 문서: 입력 중인 필드의 설명을 따라간다

**UI 페이지:** `Settings > Services > Add New Service`

Follow-along은 연결 Form과 우측 안내 문서를 **같은 field ID로 동기화하는 UI 기능**이다. 일반 tooltip이나 참고 자료 목록이 아니다. 연결 설정 단계에서 사용자가 입력 중인 항목의 설명을 바로 보게 한다.

문서는 UI에 포함된 Markdown locale asset이다. 기본 위치는 `public/locales/<언어>/<서비스 분류>/<서비스명>.md`이며, 예를 들어 MySQL 문서는 `public/locales/en-US/Database/Mysql.md`에 있다. `ServiceDocPanel`은 현재 언어 파일을 `/locales/...` 경로로 요청하고, 없으면 `en-US` 파일을 다시 요청한다.

연결 기준은 화면 Label이 아니라 Schema의 property 이름이다. MySQL Form의 `username` 입력란과 문서의 `$(id="username")`가 같은 키를 쓴다. Label은 번역될 수 있어도 이 키는 Form과 문서가 약속한 연결값으로 유지된다.

| 순서 | Form과 상태 | 문서와 화면 |
|---|---|---|
| 1 | JSON Schema에 `username` property가 있다. | `Mysql.md`에 `$(id="username")` section이 있다. |
| 2 | 사용자가 username 입력란을 focus한다. | 문서는 이미 우측 패널에 표시되어 있다. |
| 3 | `ConnectionConfigForm`의 `onFocus('username')`가 Add Service의 `activeField` state를 바꾼다. | `ServiceDocPanel`이 바뀐 `activeField`를 받는다. |
| 4 | - | `processDocMarkdown()`이 `$$section`을 `<section data-id="username">`로 바꾼다. |
| 5 | - | 같은 `data-id` section을 찾아 스크롤하고 강조한다. |

**Form과 패널의 연결:** Add Service 페이지는 focus된 field ID를 state로 보관하고, 같은 값을 우측 패널에 prop으로 전달한다.
...
**문서의 연결 표시:** 문서 작성자는 같은 field ID를 section marker에 적는다.
...
따라서 Follow-along은 별도 문서 저장소나 별도 데이터 모델이 아니다. **Form focus 이벤트의 field ID를 상태로 전달하고, 번들 Markdown을 같은 ID로 찾아 스크롤·강조하는 연결 로직**이다. ID가 다르면 Form 입력 자체는 동작하지만 우측 문서는 이동·강조되지 않는다.
...
여기서 혼동하기 쉬운 점은 코드 이름이다. 이 Provider는 OpenMetadata의 `context/UntitledUIThemeProvider/theme-provider.tsx`가 export하는 내부 `ThemeProvider`이고, 앱에서만 `UntitledUIThemeProvider`라는 별칭으로 import한다. `@untitledui/icons`에서 가져오는 컴포넌트가 아니다.
...
이 Provider의 역할은 아이콘이 아니라 **색상 설정 전달**이다. `primaryColor` 같은 `customTheme` 값을 `--tw-color-brand-600` 같은 CSS 변수로 바꾸고, 새 공유 UI의 Button·Input은 그 변수 이름을 읽어 배경·hover·선택 상태에 적용한다. 이름은 Untitled UI 스타일의 색상 체계를 따르기 때문에 붙었을 뿐, 외부 라이브러리 화면을 그대로 띄우는 경로는 아니다.
...
정리하면, 관리자가 Persona 문서에 허용된 위젯의 layout을 저장하면 `Home / My Data` 화면은 선택 Persona에 맞는 위젯 컴포넌트를 그 위치에 다시 렌더링한다.
