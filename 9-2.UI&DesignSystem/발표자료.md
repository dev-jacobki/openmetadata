...
세 폴더는 Atomic Design의 atom → molecule → organism을 그대로 구현한 엄격한 계층은 아니다. 다만 **화면을 조립할 때 `base`가 가장 아래쪽에서 많이 재사용되고, `application`이 그 위에서 `base`를 조합하는 방향은 맞다.** 예를 들어 `application/Table`은 Badge·Checkbox·Dropdown·Tooltip 같은 `base` 컴포넌트를 import한다.

`foundations`는 `application`과 나란한 “더 큰 화면 부품”도, `base` 위에 놓이는 organism도 아니다. Typography·Dot·Featured Icon처럼 여러 계층에서 공통으로 쓸 **시각적 의미 부품**을 둔 폴더다. 실제로 `base/Alert`는 `foundations/FeaturedIcon`을 쓰고, 반대로 `foundations/Typography`는 `base/Tooltip`을 쓴다. 따라서 폴더 위치만 보고 단방향 의존 관계로 설명하면 틀린다.

```text
업무 화면
  └─ application/  Table · Modal · Tabs 같은 큰 조합 부품
       └─ base/    Button · Input · Checkbox 같은 재사용 부품

base/  ↔  foundations/   Tooltip · Typography · Featured Icon처럼
                         필요한 곳에서 서로 조합될 수 있는 공통 표현 부품
```

| 분류 | 실제 역할 | Atomic Design에 굳이 비유하면 | 주의할 점 |
|---|---|---|---|
| `base` | 여러 화면에서 반복하는 입력·선택·버튼·작은 레이아웃 부품 | atom이 중심이지만 InputGroup·Card·Form처럼 작은 molecule도 섞여 있다. | “모두 한 원자”인 폴더는 아니다. |
...
| `foundations` | 글자·점·강조 아이콘처럼 공통 시각 표현을 의미 있게 만드는 부품 | Atomic Design의 organism에 대응하지 않는다. 디자인 token·표현 규칙에 가깝다. | base와 application 어디에서나 쓸 수 있으므로 “가장 상위 단계”가 아니다. |

정리하면 **`base`는 주로 하위 재사용 부품, `application`은 이를 조합한 상위 화면 부품, `foundations`는 두 계층에 가로로 적용되는 공통 표현 부품**이다. 이 분류의 목적은 조립 순서를 강제하는 것이 아니라, 새 컴포넌트를 어디에 추가하고 재사용할지 판단하게 하는 것이다. 패키지의 `components/index.ts`는 세 분류를 모두 다시 export하므로 앱에서는 필요한 컴포넌트를 바로 import한다.
...
`@openmetadata/ui-core-components`는 Ant Design이나 MUI를 한 번 더 감싼 패키지가 아니다. 클릭·키보드·focus 같은 동작과 접근성은 `react-aria-components`가 맡고, 스타일은 Tailwind의 `tw:` 접두사로 작성한다. Untitled UI는 이 새 공유 UI가 따르는 **색상 단계, 글자 크기, 간격, 둥근 모서리, 그림자, 아이콘 표현 방식의 디자인 기준**이다.

여기서 Untitled UI를 별도 화면 라이브러리나 Provider로 이해하면 안 된다. OpenMetadata는 Untitled UI 전체 컴포넌트를 가져와 화면을 그리는 대신, 다음처럼 필요한 부분을 자체 공유 UI에 반영한다.
...
`switch/case`는 “입력값이 이 값이면 이 처리를 한다”는 선택표를 코드로 적는 문법이다. `switch`에는 판별할 값 하나를 넣고, 각 `case`에는 그 값일 때의 결과를 둔다. 여기서는 “어떤 Service를 골랐는가”를 Form용 JSON Schema에 연결하는 **명시적 목록** 역할을 한다. 데이터의 유효성을 검사하거나 Form을 그리는 코드가 아니라, 여러 Schema 중 하나를 고르는 코드다.
...
Untitled UI는 사용자가 설치하거나 화면에서 고르는 네 번째 UI 라이브러리가 아니다. OpenMetadata가 새 공유 UI를 만들 때 참고하는 **디자인 규칙의 이름**이다. 여기서 디자인 규칙은 “주요 버튼의 배경색은 무엇인가”, “hover했을 때 어떤 색인가”, “입력창 테두리·본문 글자색은 무엇인가”처럼 여러 컴포넌트가 함께 써야 하는 값을 말한다.

`UntitledUIThemeProvider`는 Theme 설정값을 브라우저에서 재사용할 수 있는 이름표로 바꾼다. CSS 변수는 `--tw-color-brand-600`처럼 이름을 붙인 전역 색상 값이다. 새 공유 UI의 Button·Input은 색상을 각 파일에 직접 적지 않고 이 이름표를 읽는다.
...
따라서 “Untitled UI token을 쓴다”는 말은 새 컴포넌트마다 파란색·회색을 직접 정하지 않고, 프로젝트가 이미 정해 둔 색상·간격·글자 규칙을 재사용한다는 뜻이다.
...
### 5.5 기본 layout · 실패 시 대체값 · Persona별 배경색
