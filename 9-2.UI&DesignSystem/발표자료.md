...
| `UntitledUIThemeProvider` | OpenMetadata가 만든 Theme Provider | OpenMetadata Theme 값을 새 공유 UI용 CSS 변수로 변환. 외부 아이콘 패키지와는 별개 |

따라서 `@untitledui/icons`와 `UntitledUIThemeProvider`는 이름만 비슷할 뿐 역할이 다르다. 전자는 외부에서 가져오는 **아이콘 묶음**이고, 후자는 OpenMetadata가 만든 **색상 전달기**다. 아이콘 자체에는 Theme 설정이 없으며, Provider는 아이콘을 꾸미는 도구도 아니다.
...
여기서 `UntitledUIThemeProvider`는 외부 Untitled UI 제품이 제공하는 Provider가 아니다. **OpenMetadata가 만든 Provider**이며, 새 공유 UI가 Untitled UI 스타일의 색상 이름을 쓰기 때문에 그렇게 이름 붙었다. 외부 Untitled UI 관련 직접 의존성은 `@untitledui/icons`뿐이다. Button·Input을 포함한 화면 컴포넌트는 OpenMetadata 공유 UI 패키지가 React Aria와 Tailwind로 구현한다.

`UntitledUIThemeProvider`의 역할은 아이콘이 아니라 **색상 설정**이다. Theme 설정값을 브라우저에서 재사용할 수 있는 이름표로 바꾼다. CSS 변수는 `--tw-color-brand-600`처럼 이름을 붙인 전역 색상 값이다. 새 공유 UI의 Button·Input은 색상을 각 파일에 직접 적지 않고 이 이름표를 읽는다.
