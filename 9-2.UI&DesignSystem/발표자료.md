...
직무별 화면은 조직이 Persona 문서의 Navigation과 Landing layout을 구성해 만든다.

---

## 5. 커스터마이즈 가능한 Landing Page

Landing Page 커스터마이즈는 관리자가 Persona별로 등록된 위젯을 고르고, 카드의 위치와 크기를 layout에 저장하는 기능이다. 사용자 화면은 저장된 layout을 읽어 해당 위젯 컴포넌트를 다시 렌더링한다.
...
Pluggable Panel은 OpenMetadata에 등록된 panel 컴포넌트를 Landing layout에 추가·제거하는 구조다. 관리자는 목록에서 위젯을 고르고, 개발자는 새 위젯의 React 컴포넌트와 등록 정보를 구현한다.
...
Add Widget dialog는 `KnowledgePanel` 문서를 목록으로 읽어 위젯 키를 layout에 넣는다. 화면을 그릴 React 컴포넌트와 위젯 키 매핑은 코드에 구현되어 있어야 한다.
...
새 위젯은 React 컴포넌트, `getWidgetsFromKey()` 매핑, `KnowledgePanel` 문서가 함께 등록되어야 선택과 렌더링이 모두 동작한다.
...
`data-grid={widget}`은 위치·크기를 전달하고, `getWidgetFromKey()`는 위젯 키로 Activity Feed·KPI·My Tasks 등의 컴포넌트를 선택한다. **layout은 배치 정보이고, 위젯 컴포넌트는 구현된 화면 기능**이다. Persona별 layout은 기존 기능의 순서와 조합을 바꾼다.
...
`layout`에는 위젯 키와 위치·크기(`i`, `x`, `y`, `w`, `h`)를 저장한다. 카드의 업무 데이터는 각 React 위젯이 화면을 열 때 조회하므로, 같은 layout도 사용자와 조회 시점에 따라 내용이 달라질 수 있다. Persona 이름 자체가 위젯 데이터를 공통 필터링하지는 않으며, 위젯별 `config` 또는 구현 코드가 별도 데이터 조건을 처리한다.
